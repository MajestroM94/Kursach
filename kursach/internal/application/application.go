...
func (a app) Routes(r *httprouter.Router) {
	...
	r.GET("/analitic", a.authorized(a.Analitic))
...
func (a app) Analitic(rw http.ResponseWriter, r *http.Request, p httprouter.Params) {
	lp := filepath.Join("public", "html", "analitic.html")
	tmpl, err := template.ParseFiles(lp)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	...
	err = tmpl.ExecuteTemplate(rw, "analitic", nil)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	}
...
func (a app) Profil6(rw http.ResponseWriter, r *http.Request, p httprouter.Params) {
	userID := a.getCurrentUserID(r)
	if userID == 0 {
		http.Redirect(rw, r, "/login6", http.StatusSeeOther)
		return
	}
	...
	// Парсинг шаблона и передача данных в него
	lp := filepath.Join("public", "html", "profil6.html")
	tmpl, err := template.ParseFiles(lp)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	...
	err = tmpl.ExecuteTemplate(rw, "profil6", data)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	}
...
func (a app) Profil7(rw http.ResponseWriter, r *http.Request, p httprouter.Params) {
	userID := a.getCurrentUserID(r)
	if userID == 0 {
		http.Redirect(rw, r, "/login7", http.StatusSeeOther)
		return
	}
	...
	// Парсинг шаблона и передача данных в него
	lp := filepath.Join("public", "html", "profil7.html")
	tmpl, err := template.ParseFiles(lp)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	...
	err = tmpl.ExecuteTemplate(rw, "profil7", data)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	}
...
func (a app) Profil8(rw http.ResponseWriter, r *http.Request, p httprouter.Params) {
	userID := a.getCurrentUserID(r)
	if userID == 0 {
		http.Redirect(rw, r, "/login5", http.StatusSeeOther)
		return
	...
	// Парсинг шаблона и передача данных в него
	lp := filepath.Join("public", "html", "profil8.html")
	tmpl, err := template.ParseFiles(lp)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	...
	err = tmpl.ExecuteTemplate(rw, "profil8", data)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusBadRequest)
		return
	}
